...
def display_speed(bit_rate):
    if bit_rate < 1000:
        rate = "%s b/s" % bit_rate
    elif bit_rate >= 1000 and bit_rate < 1000000:
        rate = "%s Kb/s" % round(bit_rate/1000,2)
    elif bit_rate >= 1000000 and bit_rate < 1000000000:
        rate = "%s Mb/s" % round(bit_rate/1000000,2)
    elif bit_rate >= 1000000000:
        rate = "%s Gb/s" % round(bit_rate/1000000000,2)
    return rate
...
def get_stats(switch_members, ig_name):
    session_cookie = 'session_cookie=%s' % cookie
    headers = {"content-type": "application/json", 'Cookie': session_cookie}
    path = '/api/v1/data/controller/applications/bcf/info/statistic/interface-rate[interface/name="%s"]\
    ...
    if r_stats:
        ...
        rate = r_stats[0]['interface'][0]['rate'][0]
        ...
        rx_bit_rate = display_speed(rate['rx-bit-rate'])
        tx_bit_rate = display_speed(rate['tx-bit-rate'])
        print("%s: over last 45 seconds" % ig_name)
        output = [{"switch_name": switch_name, "interface_name": int_name, "rx_bad_vlan_pkt": rx_bad_vlan_pkt, "rx_crc_error":\
                   rx_crc_error, "rx_error": rx_error, "tx_error": tx_error, "rx_drop": rx_drop, "tx_drop": tx_drop, "rx_rate": rx_bit_rate, "tx_rate": tx_bit_rate}]
